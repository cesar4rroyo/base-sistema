...
<div class="container">
    <div class="row">
        @include ('admin.grupomenu.modalAgregar')
        @include ('admin.grupomenu.modalEditar')
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">Grupo Menu</div>
                <div class="card-body">
                    <button class="btn btn-outline-success" id="btnAddNew" title="Agregar nuevo Grupo">
                    ...
                    <div class="row mb-2" id="btnsReport">
                        <div class="table-responsive mt-4">
                            <table class="table text-center table-hover mt-4" id="grupomenu" class="display"
                                style="width:100%">
                                <thead>
                                    <tr>
                                        <th>Nombre</th>
...
document.addEventListener("DOMContentLoaded", function(event) {
    ...
    function fetchData() {
        fetch("{{route('get_grupo')}}", {
                method: 'GET',
            }).then(res => res.json())
            .then((data) => {
                $('#btnsReport').show();
                var table = $('#grupomenu').DataTable({
                    "language": {
                        "decimal": "",
                ...
                $('#grupomenu tbody').on('click', 'button', function() {
                    var action = this.className;
                    var data = table.row($(this).parents('tr')).data();
                    var id_data = data.id;
                    //opciones ver
                    if (action.includes('ver')) {
                        var urlLocal = "http://localhost/sistema/public/admin/grupomenu/show" +
                            '/' +
                            id_data;
